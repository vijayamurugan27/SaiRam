...
/* Finds the length of a list, while there is something in the list it increments N when 
there is nothing left it returns.*/ 
len([], 0). 
len([H|T], N):- len(T, X), N is X+1 . 
/*Best path, is called by shortest_path.  It sends it the paths found in a  path, distance 
format*/ 
...
/*This adds the stopping location to the visited list, adds the distance and then calls 
recursive to the next stopping location along the path */ 
...
/*When we find a path back to the starting point, make that the total distance and 
...
/* Is called, compares 2 distances. If cost is smaller than bcost, no need to go on. Cut 
it.*/ 
